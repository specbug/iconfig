...
def connect_existing_repo(url: str, branch: str) -> bool:
    ...
    try:
        ...
        if current_branch_name == branch:
            ...
            logger.info(
                f"Checking out remote branch '{branch}' and setting up tracking."
            )
            exit_code, _, stderr = run_command(
                ["git", "checkout", "-t", f"origin/{branch}"], cwd=REPO_DIR, check=False
            )
            if exit_code != 0:
            ...
            logger.warning(
                f"Branch '{branch}' not found locally or on remote 'origin'. Creating new local branch '{branch}'."
            )
            logger.warning(
                "You may need to push it to the remote manually if it's intended to be a shared branch."
            )
            exit_code, _, stderr = run_command(
                ["git", "checkout", "-b", branch], cwd=REPO_DIR, check=False
            )
...
def get_font_files_for_family(font_family_name: str) -> List[str]:
    ...
    for font_file in all_font_files:
        ...
        if (
            family_lower in font_file_lower
            or family_no_space in font_file_lower
            or family_lower.replace(" ", "-") in font_file_lower
            or family_lower.replace(" ", "_") in font_file_lower
        ):
...
def backup_fonts(
    ...
    try:
        if custom_fonts:
            ...
            for font_family in custom_fonts:
                ...
                logger.info(
                    f"Found {len(font_files)} files for font family '{font_family}'"
                )
                ...
                for font_file in font_files:
                    ...
                    if os.path.exists(font_path):
                        if dry_run:
                            logger.info(
                                f"[DRY RUN] Would copy {font_file} (from {font_family})"
                            )
                            files_copied += 1
                        else:
            ...
            logger.warning(
                "No custom fonts or include patterns specified - backing up ALL fonts"
            )
            logger.warning(
                "This may result in large backup sizes. Consider specifying custom_fonts or include_patterns."
            )
    ...
    if files_copied == 0:
        ...
        logger.info(
            f"{'Would backup' if dry_run else 'Backed up'} {files_copied} font files"
        )
...
def backup_utility(
    ...
    for path in source_paths:
        ...
        try:
            if os.path.isdir(expanded_path):
                ...
                if dry_run:
                    logger.info(
                        f"[DRY RUN] Would copy directory {expanded_path} to {dest_dir}"
                    )
                    files_copied += 1
                else:
                ...
                if dry_run:
                    logger.info(
                        f"[DRY RUN] Would copy file {expanded_path} to {backup_path}"
                    )
                    files_copied += 1
                else:
    ...
    if files_copied == 0:
        ...
        logger.info(
            f"{'Would backup' if dry_run else 'Backed up'} {files_copied} files for {utility_name}"
        )
...
def restore_utility(utility_name: str, dest_paths: list) -> bool:
    ...
    # Special handling for shell configuration
    if utility_name == "shell" and success:
        configure_shell_loader_after_restore()

    return success


def configure_shell_loader_after_restore() -> bool:
    """Configure shell to load restored aliases after restore operation."""
    logger.info("Configuring shell to load restored aliases")

    # Check if loader file exists
    loader_file = os.path.expanduser("~/.iconfig/shell/load.sh")
    if not os.path.exists(loader_file):
        logger.warning("Shell loader file not found after restore")
        return False
    ...
    if rc_file and os.path.exists(rc_file):
        ...
        if "iconfig/shell/load.sh" not in content:
            print_info(f"Adding shell alias loader to {rc_file}...")

            # Backup RC file
            backup_path = (
                f"{rc_file}.backup.{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            print_warning("Run 'source ~/.zshrc' or restart your shell to load aliases")
        else:
            logger.info("Shell loader already configured")
            print_info(
                "Shell loader already configured - aliases will be available after shell restart"
            )
    else:
        print_warning("Could not detect shell configuration file")
        print_info("To load aliases, add this line to your shell config:")
        print_info(
            '[ -f "$HOME/.iconfig/shell/load.sh" ] && source "$HOME/.iconfig/shell/load.sh"'
        )
...
def pull_changes(
    branch: Optional[str] = None, use_rebase: Optional[bool] = None
) -> bool:
    ...
    logger.info(
        f"Pulling changes from branch: {branch} (strategy: {'rebase' if use_rebase else 'merge'})"
    )

    try:
        # First, check if there are uncommitted changes
        exit_code, stdout, _ = run_command(
            ["git", "status", "--porcelain"], cwd=REPO_DIR, check=False
        )
        if exit_code == 0 and stdout.strip():
            ...
            if exit_code != 0:
                ...
                print_error(
                    "Failed to stash uncommitted changes. Please commit or stash them manually."
                )
                return False
        ...
        if exit_code != 0:
            if "CONFLICT" in stderr or "conflict" in stderr.lower():
                ...
                print_error(
                    "Conflicts detected during pull. Attempting to abort rebase..."
                )
                ...
                exit_code, _, stderr = run_command(
                    ["git", "pull", "origin", branch], cwd=REPO_DIR, check=False
                )

                if exit_code != 0:
                    logger.error(
                        f"Failed to pull changes even with merge strategy: {stderr}"
                    )
                    print_error(
                        "Failed to pull changes. Your repository may have conflicts that need manual resolution."
                    )
                    print_info(
                        "You can try resolving this manually with: cd ~/.mac-sync-wizard/repo && git pull"
                    )
                    return False
            else:
        ...
        if stashed:
            ...
            exit_code, _, stderr = run_command(
                ["git", "stash", "pop"], cwd=REPO_DIR, check=False
            )
            if exit_code != 0:
                logger.warning(f"Failed to pop stashed changes: {stderr}")
                print_warning("Failed to restore stashed changes automatically.")
                print_info(
                    "Your changes are still saved. You can restore them manually with: cd ~/.mac-sync-wizard/repo && git stash pop"
                )
...
def perform_sync(config: Dict[str, Any]) -> bool:
    ...
    if not perform_preflight_checks(config):
        ...
        print_error(
            "Pre-flight checks failed. Please resolve the issues above before syncing."
        )
        return False
    ...
    if not config.get("repository", {}).get("url"):
        ...
        print_error(
            "No repository configured. Please run 'mac-sync-wizard setup' first."
        )
        return False
    ...
    if not enabled_utilities:
        ...
        print_warning(
            "No utilities are enabled for sync. Enable utilities using 'mac-sync-wizard config'."
        )
        return True
    ...
    for utility in enabled_utilities:
            ...
            if custom_fonts or include_patterns:
                ...
                for path in utility_config["paths"]:
                    ...
                    if os.path.exists(expanded_path):
                        ...
                        size_info.append(
                            f"{os.path.basename(expanded_path)}: {size_human}"
                        )
        else:
        ...
        print(
            f"  • {'Would backup' if dry_run else 'Backing up'} {utility} ({utility_size_human})..."
        )
    ...
    if config["sync"]["auto_commit"] and not dry_run:
        ...
        if not push_changes(config["repository"]["branch"]):
            ...
            print_info(
                "You can try pushing manually later with: cd ~/.mac-sync-wizard/repo && git push"
            )
    ...
    logger.info(
        f"Sync operation completed successfully{' (DRY RUN)' if dry_run else ''}"
    )
    print_success(
        f"Successfully {'would sync' if dry_run else 'synced'} {len(enabled_utilities) - len(backup_failures)} utilities"
    )
    return True
    ...
    if config["sync"]["frequency"] > 0:
        if last_sync != "Never":
            try:
                ...
                next_sync_dt = last_sync_dt + datetime.timedelta(
                    seconds=config["sync"]["frequency"]
                )
...
def run_setup_wizard() -> bool:
    ...
    print(
        "Welcome to Mac Sync Wizard! This wizard will guide you through the setup process."
    )
    print(
        "This tool will help you synchronize your application settings and configurations using a Git repository."
    )
    print("Please ensure you have Git installed and configured on your system.")

    if not command_exists("git"):
        print_error(
            "Git is not installed or not found in your PATH. Mac Sync Wizard requires Git to function."
        )
        print_error(
            "Please install Git and ensure it's accessible from your terminal, then run the setup again."
        )
        return False
    ...
    if progress and not initial_config_exists:
        if get_yes_no(
            "It looks like a previous setup was interrupted. Would you like to resume?",
            default=True,
        ):
    ...
    if "repository" not in progress:
        ...
        if config["repository"]["url"]:
            print(
                f"Current repository: {config['repository']['url']} (branch: {config['repository']['branch']})"
            )
            if not get_yes_no(
                "Do you want to reconfigure the repository?", default=False
            ):
        ...
        if not repo_setup_success:
            print_error(
                "Repository setup failed or was skipped by the user and no prior valid configuration exists. Aborting setup."
            )
            return False
    ...
    if "utilities" not in progress:
        ...
        if not select_utilities(config):
            print_error(
                "Utility selection encountered an issue. Please review your choices."
            )
    ...
    if "sync" not in progress:
        ...
        if not configure_sync(config):
            print_error(
                "Sync behavior configuration failed. Using default sync settings."
            )
    ...
    if "notifications" not in progress:
        ...
        if not configure_notifications(config):
            print_error(
                "Notification preference configuration failed. Using default notification settings."
            )

        save_setup_progress(
            "notifications", {"level": config["notifications"]["level"]}
        )
    else:
    ...
    if not save_config(config):
        print_error(
            f"Failed to save the final configuration. Please check permissions for {CONFIG_PATH}"
        )
        return False
    ...
    if config["sync"]["frequency"] > 0:
        ...
        if get_yes_no(
            "Would you like to install/update the background sync service (LaunchAgent) for automatic syncing?",
            default=True,
        ):
            if install_launch_agent():  # install_launch_agent should be robust
                print_success(
                    "Background sync service (LaunchAgent) installed/updated successfully."
                )
            else:
                print_error(
                    "Failed to install/update background sync service. You can try again later using `install` command."
                )
        else:
            print_warning(
                "Background sync service not installed. You will need to run syncs manually."
            )
    else:
        print_info(
            "Automatic background sync is disabled (frequency set to manual). Skipping LaunchAgent setup."
        )
        # Check if an old one exists and offer to remove it
        plist_path = os.path.expanduser(
            "~/Library/LaunchAgents/com.mac-sync-wizard.plist"
        )
        if os.path.exists(plist_path):
            if get_yes_no(
                "Found an existing background sync service LaunchAgent, but automatic sync is disabled. Would you like to remove the old LaunchAgent?",
                default=True,
            ):
...
def configure_repository_wizard_step(config: Dict[str, Any]) -> bool:
    ...
    if config["repository"]["url"]:
        ...
        repo_options.append(
            (
                "skip",
                "Skip repository setup for now (not recommended, sync will not work)",
            )
        )
    ...
    if repo_choice == "new":
        ...
        if config["repository"]["url"]:
            ...
            return (
                True  # Considered success as user chose to keep existing valid config
            )
        else:
            print_warning(
                "Repository setup skipped. Syncing will not function until a repository is configured."
            )
...
def setup_new_repository(config: Dict[str, Any]) -> bool:
    ...
    while not repo_url:
        ...
        if not repo_url:
            ...
            repo_url.startswith(("http://", "https://")) or repo_url.startswith("git@")
        ):
...
def connect_existing_repository(config: Dict[str, Any]) -> bool:
    ...
    auth_type = get_choice("Select authentication type", ["ssh", "https"], "ssh")
    ...
    if init_repository(
        repo_url, branch
    ):  # init_repository now handles both new and existing
...
def select_utilities(config: Dict[str, Any]) -> bool:
    ...
    if installation_only:
        print_info(
            f"Installation-only utilities (not synced): {', '.join(installation_only)}"
        )
    ...
    for utility in syncable_utilities:
        ...
        enabled = get_yes_no(
            f"Enable sync for {utility} ({status})?", default=is_installed
        )
        ...
        if utility == "shell" and enabled:
            if get_yes_no(
                "\nWould you like to extract and sync your existing shell aliases and functions?",
                default=True,
            ):
                if setup_shell_sync():
                    print_success(
                        "Shell aliases and functions extracted and ready to sync!"
                    )
                else:
                    print_warning(
                        "Shell setup skipped - you can manually add items to ~/.iconfig/shell/aliases.sh"
                    )
            ...
            with open(rc_file, "r") as f:
                ...
                while i < len(lines):
                    ...
                    # Match function keyword style (function name { ... })
                        ...
                        if i < len(lines):
                            brace_count = "".join(func_lines).count("{") - "".join(
                                func_lines
                            ).count("}")
                            while i < len(lines) and brace_count > 0:
                                func_lines.append(lines[i].rstrip())
    ...
    if rc_file and os.path.exists(rc_file):
        ...
        if "iconfig/shell/load.sh" not in content:
            if get_yes_no(f"\nAdd shell alias loader to {rc_file}?", default=True):
                ...
                print_info(f"Backup saved to {backup_path}")
                print_info(
                    "Run 'source ~/.zshrc' or restart your shell to load aliases"
                )
        else:
...
def configure_fonts(fonts_config: Dict[str, Any]) -> None:
    ...
    print(
        "By default, ALL fonts in ~/Library/Fonts/ will be synced (can be very large)."
    )
    print(
        "You can specify font families by name (as shown in Font Book) to sync only specific fonts."
    )
    ...
    if choice == "interactive":
        ...
        if selected_fonts:
            ...
            if large_files_warning:
                ...
                if not command_exists("git-lfs"):
                    print_error(
                        "Git LFS is not installed. Large files will cause issues!"
                    )
        ...
        print_info(
            "Enter font family names as they appear in Font Book (e.g., 'Zed Plex Sans', 'SF Mono')"
        )
        print_info(
            "This will automatically include all weights and styles for each font family."
        )
        ...
        while True:
            ...
            if font_files:
                print(
                    f"  Found {len(font_files)} files: {', '.join(font_files[:3])}"
                    + (
                        f" and {len(font_files) - 3} more"
                        if len(font_files) > 3
                        else ""
                    )
                )
        ...
        print_info(
            "Enter file patterns to match fonts (e.g., 'MyCompany-*.ttf', 'Custom*.otf')"
        )
        print_info("Press Enter on an empty line when done.")
        ...
        if exclude_patterns:
            ...
            print_success(
                f"Will exclude fonts matching {len(exclude_patterns)} patterns"
            )
        else:
    ...
    if fonts_config.get("custom_fonts"):
        print_info(
            f"Font families to sync: {', '.join(fonts_config['custom_fonts'][:3])}"
            + (
                f" and {len(fonts_config['custom_fonts']) - 3} more"
                if len(fonts_config["custom_fonts"]) > 3
                else ""
            )
        )
...
def configure_sync(config: Dict[str, Any]) -> bool:
    ...
    frequency_key = get_choice(
        "Select sync frequency", [key for key, _, _ in frequency_options], "6hours"
    )
    ...
    print_success(
        f"Sync configuration saved: {frequency_desc}, auto-commit: {'enabled' if auto_commit else 'disabled'}"
    )
...
def configure_notifications(config: Dict[str, Any]) -> bool:
    ...
    level = get_choice(
        "Select notification level", [key for key, _ in notification_options], "errors"
    )
...
def setup_command(args):
    ...
    if hasattr(args, "fresh") and args.fresh:
        ...
        print_warning(
            "Fresh reinstall requested. This will remove all existing configuration and repository data."
        )

        if get_yes_no(
            "Are you sure you want to perform a fresh reinstall? This cannot be undone.",
            default=False,
        ):
            ...
            print_success(
                "Fresh reinstall preparation complete. Starting setup wizard..."
            )
        else:
...
def status_command(args):
    ...
    print(
        f"Enabled utilities: {status['enabled_utilities_count']}/{status['total_utilities_count']}"
    )
...
def install_command(args):
    ...
    logger.info(
        f"{'Installing' if args.install else 'Uninstalling'} background service"
    )
...
def help_command(args):
    ...
    print(
        "Mac Sync Wizard is a tool to synchronize your Mac utilities across multiple machines."
    )
    print("\nCommands:")
...
def restore_command(args):
    ...
    if not perform_preflight_checks(config):
        print_error(
            "Pre-flight checks failed. Please resolve the issues above before restoring."
        )
        return
    ...
    if args.utility:
        ...
        if not utilities_to_restore:
            print_warning(
                "No utilities are enabled. Enable utilities using 'mac-sync-wizard config'."
            )
            return

    print_warning(
        f"This will restore configurations for: {', '.join(utilities_to_restore)}"
    )
...
def main():
    ...
    setup_parser.add_argument(
        "--fresh",
        action="store_true",
        help="Perform a fresh reinstall, removing all existing configuration and data",
    )
    setup_parser.add_argument("--repo", help="Repository URL to use for setup")
    setup_parser.add_argument(
        "--auto-restore",
        action="store_true",
        help="Automatically restore after setup (for new machines)",
    )
    ...
    config_parser.add_argument(
        "--frequency", type=int, help="Sync frequency in seconds"
    )
    ...
    config_parser.add_argument(
        "--fonts", action="store_true", help="Configure font sync settings"
    )
    ...
    install_parser = subparsers.add_parser(
        "install", help="Install or uninstall the background service"
    )
    ...
    restore_parser = subparsers.add_parser(
        "restore", help="Restore configurations from repository"
    )
